...
uint32_t counter =0;
volatile uint32_t channel_1 = 8000;
volatile uint32_t channel_2 = 4000;
 ...
 // uint32_t last_tick = 0;
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
...
#ifdef PWM_DEBUG
	  counter =_HAL_TIM_GET_COUNTER(&htim2);
#endif
	  // Get accel angle


	   g_accel_data = mpu6050_accelerometer_calibration(&error_offset,&g_accel_data);
	   acc_roll = atan2(g_accel_data.y, g_accel_data.z) * (180.0/ M_PI);
	  ...
	  //float accel_rate = g_accel_data.y / 131.0f; // for ±250°/s sensitivity
	  ...
	  // Kalman filtered roll
	  kalman_roll_angle = Kalman_GetAngle(&kalmanRoll, acc_roll , dt);
